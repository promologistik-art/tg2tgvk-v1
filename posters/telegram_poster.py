...
class TelegramPoster:
    ...
    async def publish_post(self, queue_item: PostQueue) -> bool:
        try:
            async with AsyncSessionLocal() as session:
                result = await session.execute(select(TargetChannel).where(TargetChannel.id == queue_item.target_channel_id))
    ...
    async def _mark_failed(self, queue_item: PostQueue, error_message: str):
        async with AsyncSessionLocal() as session:
            ...
            await session.commit()
            logger.warning(f"❌ Post {queue_item.id} failed: {error_message}")